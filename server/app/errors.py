...
Any exception that is not a ``SiftError`` becomes a 500 with the code ``internal_error`` and
...
class FieldNotInSchema(SiftError):
    code = "field_not_in_schema"
    http_status = status.HTTP_422_UNPROCESSABLE_CONTENT


class Unauthorised(SiftError):
...
class UnsupportedFileType(SiftError):
...
class FileTooLarge(SiftError):
    code = "file_too_large"
    http_status = status.HTTP_413_CONTENT_TOO_LARGE


class InvalidSchemaChange(SiftError):
    code = "invalid_schema_change"
    http_status = status.HTTP_422_UNPROCESSABLE_CONTENT


class InvalidValue(SiftError):
...
class LLMUnavailable(SiftError):
    code = "llm_unavailable"
    http_status = status.HTTP_503_SERVICE_UNAVAILABLE


class LLMInvalidOutput(SiftError):
...
def register_exception_handlers(app: FastAPI) -> None:
    ...
    async def _sift(_request: Request, exc: SiftError) -> JSONResponse:
